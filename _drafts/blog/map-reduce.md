...
=================================
Map and reduce functions are convenience functions for simplifying specific list operations. Both operates on a list with a **user-defined function**. The user-defined function for map is _single-parameter_ and of _non-void return type_. On invocation on a list, map iterates over the list from left to right, and applies the user-defined function on each list element, thereby producing a new value for each element. Map keeps appending the new value produced to a new list, and returns that new list, its length being the same as the input list, at the end (**maps the list** to a new one). List _transformation_ operations can easily be viewed as map operations.
