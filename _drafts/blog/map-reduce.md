...
Now, what happens if we want to query results of aggregate operations? For example, in the collection of persons, the count of persons from a certain city? It can be modelled as a map-reduce problem with a small tweak: first, map each document to _<city, count>_ pairs with _count_ = 1; setting _count_ = 1, we pretend to assert that _the corresponding city---for the time being---contains only one person_. Next, group the resultant pairs by city as an additional step, and reduce each group---city---to a single pair containing the sum of all counts in that group. Now, after reduction, we get one pair _<city, count>_ containing the total person count for each city.

RavenDB supports [map-reduce index](https://ravendb.net/docs/article-page/4.0/csharp/indexes/map-reduce-indexes) out-of-the-box, where the map function iterates over the entire collection and transforms each document into the aggregate result for itself (the _<city, 1>_ pair); the reduce function computes the same aggregate result for each city (from the temporary results computed by map.) Here follows sample implementation of a map-reduce index for count of `Person` from each city in the `Persons` collection.
